...
interface PickListStore {
  ...
  allPickListsStats: { pending: number; completed: number; total: number } | null; // For stats display
  ...
  handleManualPrint: (shipmentId: string, pickListLinesId: number) => Promise<boolean>; // Manual print with conditional update
  ...
  handleGroupPrint: (shipmentId: string, pickListLineIds: number[]) => Promise<boolean>; // Group print with update all lines
}

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3000';
const PRINTER_BASE_URL = process.env.NEXT_PUBLIC_PRINTER_BASE_URL || 'http://localhost:4000';
...
export const usePickListStore = create<PickListStore>((set, get) => ({
  ...
  fetchPickLists: async (status: string = 'pending') => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/get-picklists?status=${status}`);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch pick lists';
  ...
  fetchPickListsStats: async () => {
    try {
      ...
      const [pendingResponse, completedResponse, allResponse] = await Promise.all([
  ...
  fetchPickListLines: async (pickListId: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch pick list lines: ${response.statusText}`);
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch pick list lines';
  ...
  executeSearch: async (term: string) => {
    ...
    console.log('Executing search for:', trimmedTerm, 'Last search:', lastSearchTerm);
  ...
  checkNextItem: async (upc: string) => {
    ...
    try {
      ...
      const matchingLines = pickListLines.filter((line) =>
        line.UPC?.toLowerCase() === upc.toLowerCase()
      );
      ...
      const nextUnchecked = matchingLines.find(line => 
      ...
      if (nextUnchecked) {
        ...
        const hasDuplicateShipment = shipmentCounts[nextUnchecked.SHIPMENT_NUMBER] > 1;

        if (hasDuplicateShipment) {
          console.log('Skipping operation: Duplicate SHIPMENT_NUMBER detected:', nextUnchecked.SHIPMENT_NUMBER);
          set({ 
            matchedUPC: nextUnchecked.UPC,
            isCheckingItem: false 
          });
          // Show alert to user about duplicate shipment
          toast.warning(`⚠️ Cannot scan/update: Multiple items found with shipment number ${nextUnchecked.SHIPMENT_NUMBER}. Printing is still allowed.`);
          return;
        }

        // Condition 1: Skip operation if SHIPPED_QTY equals QUANTITY and item is already checked
        if (nextUnchecked.SHIPPED_QTY !== null && 
        ...
        if (get().lastApiCall === nextUnchecked.SHIPMENT_ID) {
          console.log('Duplicate API call prevented for SHIPMENT_ID:', nextUnchecked.SHIPMENT_ID);
          set({ isCheckingItem: false });
          return;
        }
        
        // Call the API to update the picklist line before marking as checked
        console.log('Calling API to update SHIPMENT_ID:', nextUnchecked.SHIPMENT_ID);
        ...
        console.log('Adding to checked items:', nextUnchecked.PICK_LIST_LINES_ID);
        ...
        if (shipmentToPrint) {
          ...
          if (printSuccess) {
            console.log('Print job sent successfully for SHIPMENT_ID:', shipmentToPrint);
          } else {
            console.error('Print job failed for SHIPMENT_ID:', shipmentToPrint);
            // Remove from printed shipments if print failed
            const currentState = get();
            const updatedPrintedShipments = new Set(currentState.printedShipments);
            updatedPrintedShipments.delete(shipmentToPrint);
            set({ printedShipments: updatedPrintedShipments });
          }
        } else {
          console.log('Shipment already printed, skipping print for SHIPMENT_ID:', nextUnchecked.SHIPMENT_ID);
        }
        ...
        const hasDuplicateShipment = shipmentCounts[firstLine.SHIPMENT_NUMBER] > 1;

        if (hasDuplicateShipment) {
          console.log('Skipping reset operation: Duplicate SHIPMENT_NUMBER detected:', firstLine.SHIPMENT_NUMBER);
          set({ 
            matchedUPC: firstLine.UPC,
            isCheckingItem: false 
          });
          // Show alert to user about duplicate shipment
          toast.warning(`⚠️ Cannot scan/update: Multiple items found with shipment number ${firstLine.SHIPMENT_NUMBER}. Printing is still allowed.`);
          return;
        }
        
        // Condition 1: Skip operation if SHIPPED_QTY equals QUANTITY and item is already checked
        if (firstLine.SHIPPED_QTY !== null && 
            firstLine.QUANTITY !== null && 
            firstLine.SHIPPED_QTY === firstLine.QUANTITY &&
            checkedItems.has(firstLine.PICK_LIST_LINES_ID)) {
          console.log('Skipping reset operation: Item already completed and checked');
          set({ 
            matchedUPC: firstLine.UPC,
            isCheckingItem: false 
          });
        ...
        if (get().lastApiCall === firstLine.SHIPMENT_ID) {
          console.log('Duplicate API call prevented for SHIPMENT_ID (reset):', firstLine.SHIPMENT_ID);
          set({ isCheckingItem: false });
          return;
        }
        
        console.log('All checked, resetting. Calling API for SHIPMENT_ID:', firstLine.SHIPMENT_ID);
        ...
        console.log('All checked, resetting. New checked item:', firstLine.PICK_LIST_LINES_ID);
        ...
        // Handle printing after state updates
        if (shipmentToPrint) {
          console.log('Triggering print for SHIPMENT_ID (reset):', shipmentToPrint);
          const printSuccess = await get().handlePrint(shipmentToPrint);
          if (printSuccess) {
            console.log('Print job sent successfully for SHIPMENT_ID (reset):', shipmentToPrint);
          } else {
            console.error('Print job failed for SHIPMENT_ID (reset):', shipmentToPrint);
            // Remove from printed shipments if print failed
            const currentState = get();
            const updatedPrintedShipments = new Set(currentState.printedShipments);
            updatedPrintedShipments.delete(shipmentToPrint);
          ...
          console.log('Shipment already printed, skipping print for SHIPMENT_ID (reset):', firstLine.SHIPMENT_ID);
        }
      }
  ...
  handleManualPrint: async (shipmentId: string, pickListLinesId: number): Promise<boolean> => {
    ...
    try {
      ...
      const currentLine = pickListLines.find(line => line.PICK_LIST_LINES_ID === pickListLinesId);
      ...
      const hasDuplicateShipment = shipmentCounts[currentLine.SHIPMENT_NUMBER] > 1;
      
      // If not checked and no duplicate, update first
      if (!isAlreadyChecked && !hasDuplicateShipment) {
        console.log('Manual print: Updating shipment before printing:', shipmentId);
        ...
        if (!updateResponse.ok) {
          throw new Error(`Update API call failed: ${updateResponse.status} ${updateResponse.statusText}`);
        }
        ...
        if (updateResult.success === false) {
          throw new Error(updateResult.error || 'Update API returned failure status');
        }
      ...
      console.log('Manual print: Print request successful for shipment:', shipmentId);
      set({ loadingShipmentId: null });
      return true;
  ...
  getCurrentCheckedItem: () => {
    ...
    return pickListLines.find(line => 
  ...
  getAllItemsCompleted: () => {
    ...
    return pickListLines.every(line => 
  ...
  getCurrentPickList: () => {
    ...
    return pickLists.find(pickList => pickList.PICK_LIST_ID === currentPickListId) || null;
  },
  ...
  handleGroupPrint: async (shipmentId: string, pickListLineIds: number[]): Promise<boolean> => {
    ...
    try {
      ...
      const hasCheckedLines = pickListLineIds.some(id => checkedItems.has(id));
      const allLinesChecked = pickListLineIds.every(id => checkedItems.has(id));
      
      // If not all lines are checked, update them
      if (!allLinesChecked) {
        console.log('Group print: Updating all lines for shipment:', shipmentId);
      ...
      console.log('Group print: Print request successful for shipment:', shipmentId);
      set({ loadingShipmentId: null });
      return true;
