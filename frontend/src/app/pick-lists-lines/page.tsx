...
const PickListLinesContent = () => {
  ...
  const handleGroupPrintClick = async (
    shipmentId: string,
    lines: any[],
    ...
    try {
      ...
      const pickListLineIds = lines.map((line) =>
        parseInt(line.PICK_LIST_LINES_ID)
      );

      // Use the store's group print function
      const success = await handleGroupPrint(
        shipmentId,
        pickListLineIds,
        labelprinted
      );
      ...
      if (success) {
        toast.success(
          `✅ Labels printed successfully for shipment ${shipmentId} (${lines.length} items)!`
        );
      } else {
  ...
  const checkedCount = filteredLines.filter((line) =>
    checkedItems.has(line.PICK_LIST_LINES_ID)
  ).length;
  const completedCount = filteredLines.filter(
    (line) =>
  ...
  if (!pickListId) {
    return (
      <div className="p-6">
        ...
        <div className="text-center">❌ No PICK_LIST_ID provided in URL</div>
      </div>
    );
  }
      ...
      <div className="flex justify-center mb-6">
        <div className="w-full md:w-1/2 relative">
          ...
          <div className="text-center text-sm text-gray-500 mt-2">
            Type or paste UPC, then press{" "}
            <kbd className="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-xs font-mono">
              Enter
            </kbd>{" "}
            to execute search and clear input
          </div>
        </div>
      </div>
      ...
      {duplicateShipments.size > 0 && (
        <div className="flex justify-center mb-4">
          <div className="bg-orange-100 border border-orange-400 text-orange-700 px-4 py-3 rounded-lg flex items-center gap-2">
            ...
            <span>
              {duplicateShipments.size} shipment number(s) have Bulk Order -
              scanning/updates blocked (printing still allowed)
            </span>
          </div>
        </div>
      )}
            ...
            {Object.keys(groupedLines).length > 0 ? (
              Object.entries(groupedLines).map(([shipmentNumber, lines]) => {
                ...
                const hasDuplicateShipment =
                  duplicateShipments.has(shipmentNumber);

                // For the summary row, use the first line's data
                const summaryLine = lines[0];
                const summaryChecked = lines.some((line) =>
                  checkedItems.has(line.PICK_LIST_LINES_ID)
                );
                const allLinesChecked = lines.every((line) =>
                  checkedItems.has(line.PICK_LIST_LINES_ID)
                );
                ...
                return (
                  <React.Fragment key={shipmentNumber}>
                    {/* Summary/Group Row */}
                    <TableRow
                      className={`${summaryChecked ? "bg-green-50" : ""} ${
                      ...
                      onClick={
                        hasMultipleLines
                          ? () => toggleShipmentExpansion(shipmentNumber)
                          : undefined
                      }
                    >
                      {/* Status */}
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (
                                summaryLine.SHIPMENT_STATUS === "pending" &&
                                summaryLine.SHIPPED_QTY == null
                              ) {
                                handleGroupPrintClick(
                                  summaryLine.SHIPMENT_ID,
                                  lines,
                                  false
                                );
                              } else {
                          ...
                          >
                            ...
                            {loadingShipmentId === summaryLine.SHIPMENT_ID
                              ? "Printing..."
                              : !summaryLine.SHIPPED_QTY ||
                                Number(summaryLine.SHIPPED_QTY) === 0
                              ? "print"
                              : "label(s) printed"}
                          </Button>
                          {hasMultipleLines && (
                      ...
                      <TableCell className="select-text cursor-text">
                        {hasMultipleLines
                          ...
                          : summaryLine.PRODUCT_SKU ?? "-"}
                      </TableCell>
                      <TableCell className="select-text cursor-text">
                        {hasMultipleLines
                          ? "Multiple products"
                          : summaryLine.PRODUCT_NAME ?? "-"}
                      </TableCell>
                      ...
                      lines.map((line, idx) => {
                        ...
                        return (
                          <TableRow
                            ...
                            } bg-gray-25 border-l-2 border-gray-300 ml-4`}
                          >
                            {/* Status */}
